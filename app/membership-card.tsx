...
type Member = { name:string; token:string; photo:string; plan:string; expiresAt:string; accessStatus:string; cardVariant?:number };
...
export function MembershipCard({client, qr, cardRef}: {client:Member; qr:string; cardRef?:Ref<HTMLDivElement>}) {
  ...
  return <div className="membership-card-frame">
    <div className="membership-card" ref={cardRef} data-animal={animal}>
      <img className="membership-watermark" src={`/card-animals/${animal}.png`} alt="" aria-hidden="true"/>
      ...
      <div className="membership-person">
        {client.photo ? <img className="membership-photo" src={client.photo} alt={`Foto de ${client.name}`}/> : <span className="membership-photo membership-initials">{client.name.split(/\s+/).map(n=>n[0]).slice(0,2).join("")}</span>}
      ...
      <div className="membership-lower">
        <div className="membership-details">
          ...
          <p className="membership-slogan">TU PRÓXIMO NIVEL<strong>EMPIEZA AQUÍ</strong></p>
        ...
        <div className="membership-qr-panel"><div className="membership-qr">{qr ? <img src={qr} alt={`QR único de ${client.name}`}/> : <span>Preparando QR…</span>}</div><p><span aria-hidden="true">⌗</span>ESCANEA PARA<br/>REGISTRAR INGRESO</p></div>
      </div>
    </div>
  </div>;
}
